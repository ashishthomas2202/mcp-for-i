...
async function migrateConfigFile(input) {
    let changed = false;
...
async function normalizeConnections(value) {
    ...
    for (const [index, entry] of value.entries()) {
        ...
        const settings = isPlainObject(entry.settings) ? entry.settings : undefined;
        const profiles = Array.isArray(entry.profiles) ? entry.profiles : undefined;
        ...
        if (privateKeyPath)
            normalized.privateKeyPath = privateKeyPath;
        if (settings)
            normalized.settings = settings;
        if (profiles)
            normalized.profiles = profiles;
        if (currentProfile)
            normalized.currentProfile = currentProfile;
        if (policy)
            normalized.policy = policy;
        if (keepaliveInterval !== undefined)
            normalized.keepaliveInterval = keepaliveInterval;
        if (readyTimeout !== undefined)
            normalized.readyTimeout = readyTimeout;
        if (sshDebug !== undefined)
            normalized.sshDebug = sshDebug;
...
function normalizeActions(value) {
    ...
    const filtered = value.filter(isPlainObject);
...
function normalizePolicy(value) {
    ...
    const normalizedProfile = profile === "read-only" || profile === "power-user" || profile === "guarded" ? profile : "guarded";
    const policy = { ...value, profile: normalizedProfile };
    if (!Array.isArray(policy.requireApprovalFor))
        delete policy.requireApprovalFor;
    if (!Array.isArray(policy.allowCommands))
        delete policy.allowCommands;
    if (!Array.isArray(policy.denyCommands))
        delete policy.denyCommands;
    if (!Array.isArray(policy.allowLibraries))
        delete policy.allowLibraries;
    if (!Array.isArray(policy.denyLibraries))
        delete policy.denyLibraries;
    if (!Array.isArray(policy.allowPaths))
        delete policy.allowPaths;
    if (!Array.isArray(policy.denyPaths))
        delete policy.denyPaths;
    return policy;
}
async function sanitizeConnection(connection) {
...
function uniqueConnectionName(baseName, seen) {
...
async function writeFileWithRetry(filePath, content, retries = 5, delayMs = 40) {
